...
### 4.1 ❓ Внешние календарные интеграции нужны?

**Решение:** ❌ **Нет, используем только внутреннее расписание**

**Обоснование:**
- Убираем внешние зависимости и риски недоступности.
- Вся доступность слотов хранится и рассчитывается в продукте.

**Реализация:**
- `availability_slots` и `appointments` в БД.
- Резервация и конфликты решаются внутри продукта.
- См. техспеку: `docs/tech-specs/Remove-Google-Calendar-Integration-Spec.md`.
...
- [x] Внешние календарные интеграции? → **Нет, внутреннее расписание**
...
- Внутреннее расписание (без Google Calendar)
- ЮKassa API (платежи)
- Telegram Bot API
...
- Интеграции (ЮKassa, Telegram Bot API)
...
- Расписание/слоты (внутренняя реализация)
