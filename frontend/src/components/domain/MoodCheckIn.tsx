...
const moods = [
  { value: 'great', label: 'Отлично', icon: Smile, color: 'text-green-600' },
  { value: 'good', label: 'Хорошо', icon: Heart, color: 'text-primary' },
  { value: 'okay', label: 'Нормально', icon: Meh, color: 'text-yellow-600' },
  { value: 'bad', label: 'Плохо', icon: Frown, color: 'text-orange-600' },
  ...
  return (
    <Card className="w-full max-w-2xl">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-5 gap-3">
          {moods.map((mood) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-8 h-8 ${isSelected ? mood.color : 'text-muted-foreground'}`}
                  aria-hidden="true"
                />
                <span
        ...
        {/* Save Button */}
        <Button className="w-full" disabled={!selectedMood} onClick={handleSave}>
          Сохранить
        </Button>
        ...
        <div className="p-4 bg-muted/50 rounded-lg text-center">
          <p className="text-sm text-muted-foreground">
            🔥 Вы отслеживаете настроение <strong className="text-foreground">7 дней</strong>{' '}
            подряд!
          </p>
        </div>
      </CardContent>
