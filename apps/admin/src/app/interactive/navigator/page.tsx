...
const statusBadgeClasses: Record<NavigatorDefinition['status'], string> = {
  published: 'border-success/30 bg-success/10 text-success',
  draft: 'border-warning/30 bg-warning/10 text-warning',
  archived: 'border-muted text-muted-foreground bg-muted/40',
};

export default function NavigatorListPage() {
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<NavigatorDefinition[]>([]);
  ...
  const fetchDefinitions = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/admin/interactive/definitions?type=navigator', {
        credentials: 'include',
      });
      if (!response.ok) {
        throw new Error('Не удалось загрузить навигаторы');
      }
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-foreground">Навигатор состояния</h1>
          </div>
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <Button asChild variant="link" className="px-0">
                          <Link href={`/interactive/navigator/${item.id}`}>Открыть редактор</Link>
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
