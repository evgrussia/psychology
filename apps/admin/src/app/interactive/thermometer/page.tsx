      ...
      {loading ? (
        ...
        <div className="p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold">Термометр ресурса</h1>
              <p className="text-sm text-gray-500">Конфигурации THERMOMETER (шкалы, пороги, тексты результатов).</p>
            </div>
          </div>
            ...
            {items.map((item) => (
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <Link href={`/interactive/thermometer/${item.id}`} className="text-indigo-600 hover:text-indigo-900">
                    Редактировать
                  </Link>
                </td>
              </tr>
