...
export class SubmitAnonymousQuestionUseCase {
  ...
  async execute(dto: SubmitAnonymousQuestionRequestDto): Promise<SubmitAnonymousQuestionResponseDto> {
    const triggerFlags = this.detectTriggers(dto.text, dto.contactValue);
  ...
  private detectTriggers(text: string, contactValue?: string): UgcTriggerFlag[] {
    const flags = new Set<UgcTriggerFlag>();
    const content = `${text} ${contactValue ?? ''}`.toLowerCase();

    const crisisKeywords = ['суицид', 'убить себя', 'покончить с собой', 'не хочу жить', 'самоповреждение'];
    ...
    const medicalKeywords = ['диагноз', 'лечение', 'психиатр', 'медицин', 'лекарств'];
